...
当前节点包偏实用向：快速创建 latent、批量流程中人工筛图暂停、提示词文本批量替换。
...
```text
上游批量生成图片 -> EU_图片筛选暂停 -> 后续放大 / 修脸 / 保存 / 二次处理
```
...
- `全不选`：清空当前选择。
- `反选`：把已选和未选状态互换。
- `取消流程`：取消当前流程，节点会报错并阻止下游继续执行。
- `继续所选`：确认选择并继续执行下游节点。

注意：

- 这个节点会阻塞当前工作流，直到你点击继续、取消，或者等待超时。
...
- `EU_图片筛选暂停` 适合人工参与的工作流，不适合完全无人值守的批量任务。
