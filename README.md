...
当前节点包偏实用向：快速创建 latent、批量流程中人工筛图、图片批次/列表转换、提示词文本拼接和批量替换。

## 本版更新

- `EU_图片筛选` 增加对新版 ComfyUI 列表执行的兼容处理，可接收单个 `IMAGE/LATENT` batch，也可接收上游节点传来的列表结果，并在筛选前合并为可预览的 batch。
- `EU_图片筛选` 对新版 ComfyUI 额外传入的 `unique_id` 做了兼容，避免 `IS_CHANGED()` 参数不匹配产生警告。
- `EU_图片筛选` 增强了图片张量形状处理，兼容 `[B,H,W,C]`、单张 `[H,W,C]`，以及部分新版流程里可能出现的单帧 5D 图片张量。
- 新增 `EU_图片批次转列表`，用于把一个 `IMAGE` batch 拆成 ComfyUI 列表执行，适合需要把批次图片改为逐张处理的节点链路。
- `EU_图片筛选` 前端已支持图片预览缩放，可用 `- / 滑块 / +` 调整弹窗里的缩略图大小。
...
一个用于把 `IMAGE` batch 拆成 ComfyUI 列表执行的小工具节点。

它不修改图片内容，只改变下游执行方式：输入是一组批次图片，输出会变成多次单图执行。适合在某些不适合直接吃多图 batch，或者希望每张图独立走后续流程的节点前使用。

常见用法：

```text
图片 batch -> EU_图片批次转列表 -> 需要逐张处理的后续节点
```

输入：

- `images`：需要拆分的图片 batch，标准形状为 `[B,H,W,C]`。

输出：

- `images`：ComfyUI 列表形式的单张图片，每一项仍是标准 `IMAGE[1,H,W,C]`。

使用建议：

- 如果后续节点可以稳定处理 batch，可以不使用这个节点；只有需要逐张执行时再接入。
- 对需要一张图对应一次独立处理的链路，建议把批次转列表节点放在这段链路的入口前。
- 如果后面有 `ColorMatch` 之类需要参考原图的节点，参考图输入也建议使用同一个列表输出，避免一边是 batch、一边是列表造成数量对不上。
- 这个节点不会生成更多图片，只是把已有 batch 拆开让下游逐张执行。
